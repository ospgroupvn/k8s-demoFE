...
import { reportOrgViolation } from "@/service/notaryOrg";
...
const DEFAULT_COLUMNS = [
  { index: 2, label: "Kỷ luật" },
  { index: 3, label: "Xử lý vi phạm hành chính" },
  { index: 4, label: "Truy cứu trách nhiệm hình sự" },
];

const ReportOrgViolation = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.CONG_CHUNG.VI_PHAM_TC, searchParam],
    queryFn: reportOrgViolation,
    refetchOnWindowFocus: false,
  });

  const queryAll = useQuery({
    queryKey: [
      QUERY_KEY.CONG_CHUNG.VI_PHAM_TC,
  ...
  //   queryKey: [
  //     QUERY_KEY.CONG_CHUNG.VI_PHAM_TC,
  //     { aTypes: ["3"], numberPerPage: 100, pageNumber: 1 },
  //   ],
  //   queryFn: reportOrgViolation,
  //   refetchOnWindowFocus: false,
  ...
  const columns: ColumnDef<ReportItem>[] = [
    ...
    {
      accessorKey: "col_2",
      header: () => <div className="line-clamp-3 min-w-[120px]">Kỷ luật</div>,
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
  ...
  const onExportClick = () => {
    ...
    let url =
      ...
      `/private/tccc/exportExcelReportOperationViolationOrganization?`;
    ...
    const filename = `${format(
      new Date(),
      "dd-MM-yyyy"
    )}_Báo cáo xử lý vi phạm của tổ chức HNCC.xlsx`;
  ...
  return (
    <>
      <h1 className="text-lg font-bold mb-6 uppercase">
        Báo cáo xử lý vi phạm của tổ chức hành nghề công chứng
      </h1>
      ...
      <Tabs defaultValue="chartTab" className="mt-3">
        ...
        >
          <p className="font-bold mb-4 w-full">
            Thống kê xử lý vi phạm hành chính của các tổ chức hành nghề công
            chứng trên các tỉnh, thành phố
          </p>
...
export default ReportOrgViolation;
