...
import { reportCCVViolation } from "@/service/notaryOrg";
...
const ReportCCVViolation = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.CONG_CHUNG.VI_PHAM_CCV, searchParam],
    queryFn: reportCCVViolation,
    refetchOnWindowFocus: false,
  });

  const queryAll = useQuery({
    queryKey: [
      QUERY_KEY.CONG_CHUNG.VI_PHAM_CCV,
  ...
  const onExportClick = () => {
    ...
    let url =
      ...
      `/private/tccc/exportExcelReportOperationViolation?`;
    ...
    const filename = `${format(
      new Date(),
      "dd-MM-yyyy"
    )}_Báo cáo xử lý vi phạm của CCV.xlsx`;
  ...
  return (
    <>
      <h1 className="text-lg font-bold mb-6 uppercase">
        BÁO CÁO XỬ LÝ VI PHẠM CỦA CÔNG CHỨNG VIÊN
      </h1>
      ...
      <Tabs defaultValue="chartTab" className="mt-3">
        ...
        >
          <p className="font-bold mb-4 w-full">
            Thống kê xử lý vi phạm hành chính của CCV trên các tỉnh, thành phố
          </p>
...
export default ReportCCVViolation;
