...
import { reportNotaryOrg } from "@/service/notaryOrg";
...
const DEFAULT_COLUMNS = [
  { index: 2, label: "Số PCC đang hoạt động" },
  { index: 3, label: "Số PCC bị giải thể" },
  { index: 4, label: "Số PCC chuyển đổi" },
  { index: 5, label: "Số VPCC đã thành lập" },
  { index: 6, label: "Số VPCC đang hoạt động" },
  { index: 7, label: "Số VPCC chấm dứt hoạt động" },
];
...
const ReportOrg = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.CONG_CHUNG.BAO_CAO_TO_CHUC, searchParam],
    queryFn: reportNotaryOrg,
    refetchOnWindowFocus: false,
  });

  const queryAll = useQuery({
    queryKey: [
      QUERY_KEY.CONG_CHUNG.BAO_CAO_TO_CHUC,
      { ...allParams, numberPerPage: 100, pageNumber: 1 },
    ],
    queryFn: reportNotaryOrg,
    refetchOnWindowFocus: false,
    // enabled: searchParam.aTypes?.length === 1,
  });
  ...
  //   queryKey: [
  //     QUERY_KEY.CONG_CHUNG.BAO_CAO_TO_CHUC,
  //     { aTypes: ["2", "6"], numberPerPage: 100, pageNumber: 1 },
  //   ],
  //   queryFn: reportNotaryOrg,
  //   refetchOnWindowFocus: false,
  ...
  //         name: item.col_1,
  //         value: (
  //           Number(item.col_2 || "0") + Number(item.col_6 || "0")
  //         ).toString(),
  //       });
  //     }
  //   });
  ...
  const columns: ColumnDef<ReportItem>[] = [
    ...
    {
      accessorKey: "activePCC",
      header: "Số PCC đang hoạt động",
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "dissolvePCC",
      header: "Số PCC bị giải thể",
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "changePCC",
      header: "Số PCC chuyển đổi",
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "createdVPCC",
      header: "Số VPCC đã thành lập",
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "activeVPCC",
      header: "Số VPCC đang hoạt động",
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "inactiveVPCC",
      header: "Số VPCC chấm dứt hoạt động",
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
  ...
  const onExportClick = () => {
    ...
    let url =
      ...
      `/private/tccc/exportExcelReportOrganizationNotary?`;
  ...
  return (
    <>
      <h1 className="text-lg font-bold mb-6">
        BÁO CÁO SỐ LIỆU TỔ CHỨC HÀNH NGHỀ CÔNG CHỨNG
      </h1>
      ...
      <Tabs defaultValue="chartTab" className="mt-3">
        ...
        >
          <p className="font-bold mb-4 w-full">
            Biểu đồ số liệu tổ chức hành nghề công chứng
          </p>
