...
const DEFAULT_COLUMNS = [
  { index: 2, label: "Trung tâm dịch vụ đấu giá tài sản" },
  { index: 3, label: "Doanh nghiệp đấu giá tư nhân" },
  { index: 4, label: "Công ty đấu giá hợp danh" },
  { index: 5, label: "Chi nhánh DN đấu giá tài sản" },
  { index: 6, label: "VAMC" },
];
...
const ReportOrgList = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.DAU_GIA.BAO_CAO_TO_CHUC, searchParam],
    queryFn: reportAuctionOrg,
    refetchOnWindowFocus: false,
  });
  ...
  //   queryAll?.data?.data?.items?.forEach((item, index) => {
  //     if (index > 0) {
  //       newData.push({
  //         name: item.col_1,
  //         value: (
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    values: {
      ...
      org:
        orgQuery?.data?.map((item) => item.provinceCode?.toString() || "") ||
        [],
    },
  });
  ...
  const columns: ColumnDef<ReportItem>[] = [
    ...
    },
    {
      accessorKey: "col_2",
      header: () => (
        <div className="max-sm:min-w-[100px]">
          Trung tâm dịch vụ đấu giá tài sản
        </div>
      ),
      cell: ({ row }) => (
        <div className={cn(row.index === 0 ? "font-bold" : "", "text-center")}>
          {row.original.col_2 || "-"}
        </div>
      ),
      enableHiding:
    ...
    },
    {
      accessorKey: "col_3",
      header: () => (
        <div className="max-sm:min-w-[100px]">Doanh nghiệp đấu giá tư nhân</div>
      ),
      cell: ({ row }) => (
    ...
    },
    {
      accessorKey: "col_4",
      header: () => (
        <div className="max-sm:min-w-[100px]">Công ty đấu giá hợp danh</div>
      ),
      cell: ({ row }) => (
        <div className={cn(row.index === 0 ? "font-bold" : "")}>
          {row.original.col_4 || "-"}
    ...
    },
    {
      accessorKey: "col_5",
      header: () => (
        <div className="max-sm:min-w-[100px]">Chi nhánh DN đấu giá tài sản</div>
      ),
      cell: ({ row }) => (
        <div className={cn(row.index === 0 ? "font-bold" : "")}>
          {row.original.col_5 || "-"}
    ...
    {
      accessorKey: "col_6",
      header: () => <div className="max-sm:min-w-[100px]">VAMC</div>,
      cell: ({ row }) => (
        <div className={cn(row.index === 0 ? "font-bold" : "")}>
          {row.original.col_6 || "-"}
        </div>
  ...
  const onExportClick = () => {
    ...
    let url =
      ...
      `/private/dgts/report-quantity-organization-auction/export?`;
    ...
    const filename = `${format(
      new Date(),
      "dd-MM-yyyy"
    )}_Báo cáo số liệu tổ chức HNĐG.xlsx`;
  ...
  return (
    <>
      <h1 className="text-lg font-bold mb-6">
        THỐNG KÊ SỐ LƯỢNG TỔ CHỨC HÀNH NGHỀ ĐẤU GIÁ
      </h1>
      ...
      <Tabs defaultValue="chartTab" className="mt-3">
        ...
        >
          <p className="font-bold mb-4 w-full">
            Biểu đồ số lượng tổ chức hành nghề đấu giá
          </p>
