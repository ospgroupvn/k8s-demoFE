...
import { reportAuctionOperation } from "@/service/auctionOrg";
...
const DEFAULT_COLUMNS = [
  { index: 2, label: "Thành lập/cấp mới" },
  { index: 3, label: "Cấp lại giấy ĐKHD" },
  { index: 4, label: "Cấp giấy ĐKHĐ" },
  { index: 5, label: "Quyết định thành lập" },
  { index: 6, label: "Thay đổi thông tin" },
  { index: 7, label: "Chuyển đổi TT sang" },
  { index: 8, label: "Thu hồi ĐKHĐ" },
  { index: 9, label: "Sáp nhập" },
  { index: 10, label: "Giải thể" },
  { index: 11, label: "Hợp nhất" },
  { index: 12, label: "Phá sản" },
];
...
const ReportOperation = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.DAU_GIA.TINH_HINH_HOAT_DONG, searchParam],
    queryFn: reportAuctionOperation,
    refetchOnWindowFocus: false,
  });

  const queryAll = useQuery({
    queryKey: [
      QUERY_KEY.DAU_GIA.TINH_HINH_HOAT_DONG,
  ...
  //   queryKey: [
  //     QUERY_KEY.DAU_GIA.TINH_HINH_HOAT_DONG,
  //     { ...searchParam, aTypes: ["2"], numberPerPage: 100, pageNumber: 1 },
  //   ],
  //   queryFn: reportAuctionOperation,
  //   refetchOnWindowFocus: false,
  ...
  const columns: ColumnDef<ReportItem>[] = [
    ...
    {
      accessorKey: "col_2",
      header: () => <div>Thành lập/cấp mới</div>,
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "col_3",
      header: () => <div>Cấp lại giấy ĐKHD</div>,
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "col_5",
      header: () => <div>Quyết định thành lập</div>,
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "col_6",
      header: () => <div>Thay đổi thông tin</div>,
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
    ...
    {
      accessorKey: "col_7",
      header: () => <div>Chuyển đổi TT sang</div>,
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
  ...
  const onExportClick = () => {
    ...
    let url =
      ...
      `/private/dgts/report-operation-organization-auction/export?`;
    ...
    const filename = `${format(
      new Date(),
      "dd-MM-yyyy"
    )}_Báo cáo tình hình hoạt động tổ chức HNĐG.xlsx`;
  ...
  return (
    <>
      <h1 className="text-lg font-bold mb-6 uppercase">
        Báo cáo thống kê tình hình hoạt động tổ chức hành nghề đấu giá
      </h1>
      ...
      <Tabs defaultValue="chartTab" className="mt-3">
        ...
        >
          <p className="font-bold mb-4 w-full">
            Biểu đồ tình hình hoạt động của tổ chức hành nghề đấu giá
          </p>
