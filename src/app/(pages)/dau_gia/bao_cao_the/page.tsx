...
import { reportAuctionCard } from "@/service/auctionOrg";
...
const DEFAULT_COLUMNS = [
  { index: 2, label: "Cấp mới CCHN đấu giá" },
  { index: 3, label: "Cấp lại CCHN đấu giá" },
  { index: 4, label: "Thu hồi CCHN đấu giá" },
  { index: 5, label: "Cấp mới thẻ ĐGV" },
  { index: 6, label: "Cấp lại thẻ ĐGV" },
  { index: 7, label: "Thu hồi thẻ ĐGV" },
];
...
const ReportCard = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.DAU_GIA.BAO_CAO_THE, searchParam],
    queryFn: reportAuctionCard,
    refetchOnWindowFocus: false,
  });

  const queryAll = useQuery({
    queryKey: [
      QUERY_KEY.DAU_GIA.BAO_CAO_THE,
  ...
  const columns: ColumnDef<ReportItem>[] = [
    ...
    {
      accessorKey: "col_2",
      header: "Cấp mới CCHN đấu giá",
      cell: ({ row }) => (
        <div
          className={cn(
            row.index === 0 ? "font-bold" : "",
            "max-sm:max-w-[150px] max-sm:w-max"
          )}
        >
          {row.original.col_2 || "-"}
        </div>
  ...
  const onExportClick = () => {
    ...
    let url =
      ...
      `/private/dgts/report-auction-card/export?`;
    ...
    const filename = `${format(
      new Date(),
      "dd-MM-yyyy"
    )}_Báo cáo tình hình quản lý thẻ Đấu giá viên.xlsx`;
  ...
  return (
    <>
      <h1 className="text-lg font-bold mb-6">
        BÁO CÁO THỐNG KÊ TÌNH HÌNH QUẢN LÝ THẺ ĐẤU GIÁ VIÊN
      </h1>
      ...
      <Tabs defaultValue="chartTab" className="mt-3">
        ...
        >
          <p className="font-bold mb-4 w-full">
            Biểu đồ tình hình quản lý thẻ Đấu giá viên
          </p>
