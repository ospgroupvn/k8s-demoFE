...
import { reportNoticeAuctionAsset } from "@/service/auctionOrg";
...
const DEFAULT_COLUMNS = [
  { index: 2, label: "Thông báo kết quả lựa chọn có nội dung không phù hợp" },
  { index: 3, label: "Thông báo kết quả lựa chọn đã công khai" },
  { index: 4, label: "Thông báo việc đấu giá chờ duyệt" },
  { index: 5, label: "Thông báo việc đấu giá đã công khai" },
];

const ReportNoticeAsset = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.DAU_GIA.THONG_BAO_LUA_CHON, searchParam],
    queryFn: reportNoticeAuctionAsset,
    refetchOnWindowFocus: false,
  });

  const queryAll = useQuery({
    queryKey: [
      QUERY_KEY.DAU_GIA.THONG_BAO_LUA_CHON,
  ...
  //   queryKey: [
  //     QUERY_KEY.DAU_GIA.THONG_BAO_LUA_CHON,
  //     { aTypes: ["5"], numberPerPage: 100, pageNumber: 1 },
  //   ],
  //   queryFn: reportNoticeAuctionAsset,
  //   refetchOnWindowFocus: false,
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    values: {
      aTypes: ["3", "5"],
      org: provinceQuery?.data?.map((item) => item.id?.toString() || "") || [],
    },
  });
  ...
  const columns: ColumnDef<ReportItem>[] = [
    ...
    //       Thông báo kết quả lựa chọn có nội dung không phù hợp
    //     </div>
    //   ),
    //   cell: ({ row }) => (
    ...
    // },
    {
      ...
      header: () => (
        <div className="min-w-[100px]">
          Thông báo kết quả lựa chọn đã công khai
        </div>
      ),
      cell: ({ row }) => (
        <div className={cn(row.index === 0 ? "font-bold" : "")}>
    ...
    //   accessorKey: "col_4",
    //   header: () => (
    //     <div className="min-w-[100px]">Thông báo việc đấu giá chờ duyệt</div>
    //   ),
    //   cell: ({ row }) => (
    //     <div className={cn(row.index === 0 ? "font-bold" : "")}>
    //       {row.original.col_4 || "-"}
    ...
    // },
    {
      accessorKey: "col_5",
      header: () => (
        <div className="min-w-[100px]">Thông báo việc đấu giá đã công khai</div>
      ),
      cell: ({ row }) => (
        <div className={cn(row.index === 0 ? "font-bold" : "")}>
          {row.original.col_5 || "-"}
  ...
  const onExportClick = () => {
    ...
    let url =
      ...
      `/private/dgts/report-notice-auction-asset/export?`;
    ...
    const filename = `${format(
      new Date(),
      "dd-MM-yyyy"
    )}_Báo cáo tổng hợp số liệu thông báo lựa chọn tổ chức ĐGTS.xlsx`;
  ...
  return (
    <>
      <h1 className="text-lg font-bold mb-6">
        BÁO CÁO TỔNG HỢP SỐ LIỆU THÔNG BÁO LỰA CHỌN TỔ CHỨC ĐẤU GIÁ TÀI SẢN
      </h1>
            ...
            render={({ field }) => (
              <FormItem className="flex flex-col items-left col-span-2 max-sm:col-span-6">
                <MultiSelect
                  ...
                  value={field.value}
                  data={DEFAULT_COLUMNS?.filter(
                    (item) => item.index !== 2 && item.index !== 4
                  )?.map((item) => ({
      ...
      <Tabs defaultValue="chartTab" className="mt-3">
        ...
        >
          <p className="font-bold mb-4 w-full">
            Biểu đồ tổng hợp số liệu thông báo lựa chọn tổ chức đấu giá tài sản
          </p>
...
export default ReportNoticeAsset;
