...
const LawyerOrgForeignList = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.LUAT_SU.TO_CHUC_NUOC_NGOAI, searchParam],
  ...
  const columns: ColumnDef<LawyerOrgItem>[] = [
    {
      ...
      cell: ({ row }) => (
        <div
          className="text-left whitespace-normal text-default-blue cursor-pointer min-w-[300px] line-clamp-3"
          onClick={() => {
            setIsEdit(false);
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div
          className="text-left min-w-[300px] line-clamp-3 whitespace-normal"
    ...
    {
      accessorKey: "registrationLicenseIssueDate",
      header: "Ngày cấp GTPL",
      cell: ({ row }) => (
        <div className="text-left">
          {row.original.registrationLicenseIssueDate
            ? format(row.original.registrationLicenseIssueDate, "dd/MM/yyyy")
            : "-"}
        </div>
      ),
    },
...
export default LawyerOrgForeignList;
