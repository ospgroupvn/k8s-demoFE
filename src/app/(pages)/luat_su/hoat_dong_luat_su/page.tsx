...
import { reportLawyer } from "@/service/lawyer";
import { ReportItemLawyerType } from "@/types/congChung";
import { LawyerActivityReportItem, LawyerReportParams } from "@/types/luatSu";
...
const DEFAULT_COLUMNS = [
  { index: 2, label: "Đang hành nghề" },
  { index: 3, label: "Đã bị thu hồi CCHNLS" },
  { index: 4, label: "Chưa cấp thẻ Luật sư" },
  { index: 5, label: "Đã cấp thẻ Luật sư" },
  { index: 6, label: "Đã thu hồi GPHN" },
  { index: 7, label: "Đã chấm dứt hành nghề" },
];

const ReportLawyerActivity = () => {
  ...
  const query = useQuery({
    queryKey: [QUERY_KEY.LUAT_SU.HOAT_DONG_LUAT_SU, searchParam],
    queryFn: reportLawyer,
    refetchOnWindowFocus: false,
    enabled:
  ...
  const queryAll = useQuery({
    queryKey: [
      QUERY_KEY.LUAT_SU.HOAT_DONG_LUAT_SU_ALL,
    ...
    return (
      data?.map((item) => {
        return {
          ...
          col_3: item?.totalRevoke?.toString(),
          col_4: item?.totalNotCard?.toString(),
          col_5: item?.totalCard?.toString(),
          col_6: item?.totalRevoke?.toString(),
          col_7: item?.totalRevoke?.toString(),
        };
      }) || []
    );
  ...
  const columns: ColumnDef<LawyerActivityReportItem>[] = [
    {
      accessorKey: "STT",
      header: "STT",
      cell: ({ row }) => {
  ...
  const onExportClick = () => {
    ...
    const url =
      process.env.NEXT_PUBLIC_API_URL + `/private/lstc/report/law-active/excel`;

    const filename = `${format(
      new Date(),
      "dd-MM-yyyy"
    )}_Báo cáo tình hình hoạt động của Luật sư.xlsx`;
    ...
    <>
      <div className="flex justify-between items-center">
        <h1 className="text-lg font-bold mb-6 uppercase">
          TÌNH HÌNH HOẠT ĐỘNG CỦA LUẬT SƯ
        </h1>
      </div>
      ...
      <Tabs defaultValue="chartTab" className="mt-3">
        ...
        >
          <p className="font-bold mb-4 w-full">
            Biểu đồ tình hình hoạt động Luật sư
          </p>
...
export default ReportLawyerActivity;
